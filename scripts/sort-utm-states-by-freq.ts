...
type Stats<State extends string> = {
    ...
    freqStats: Partial<Record<State, {
        freq: number;
        cumFreq: number;
    }>>;
}
...
function getStats<State extends string, Symbol extends string>(
    ...
    stateCounts[doubleSimulator.state] = (stateCounts[doubleSimulator.state] || 0) + 1;
    ...
    const  freqStatsObj: Partial<Record<State, { freq: number; cumFreq: number }>> = {};
...
while (true) {
    ...
    if (lastStats){
        ...
        const lastFreq = (st: MyUtmState) => lastStatsConst.freqStats[st]?.freq ?? 0;
        const newFreq = (st: MyUtmState) => stats.freqStats[st]?.freq ?? 0;
        const changed = spec.allStates.filter(st => (percentageDiff(lastFreq(st), newFreq(st)) > 1) && (newFreq(st) > 1/(10*spec.allStates.length)));
        console.log(`${changed.length} states changed:`)
        for (const st of changed) {
            console.log(`  ${st.padEnd(20)} ${lastFreq(st).toFixed(6)} -> ${newFreq(st).toFixed(6)} (${percentageDiff(lastFreq(st), newFreq(st)).toFixed(2)}%)`);
        }
        if (changed.length === 0) {
            console.log('reached fixed point, or close enough')
            break;
        }
        spec = {...spec, allStates: spec.allStates.filter(st => !changed.includes(st))};
    }
    lastStats = stats;
